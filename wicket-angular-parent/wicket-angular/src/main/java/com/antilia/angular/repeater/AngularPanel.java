 ...
 *
 */
public abstract class AngularPanel extends Panel implements IResourceListener {
	 ...
	 * @param id
	 */
	public AngularPanel(String id, String scope) {
		super(id);
		this.scope = !Strings.isEmpty(scope)? scope: "AngularPanel"+getMarkupId();
	 ...
	 * @param model
	 */
	public AngularPanel(String id, IModel<?> model, String scope) {
		super(id, model);
		this.scope = !Strings.isEmpty(scope)? scope: "AngularPanel"+getMarkupId();
		this.add(new AttributeModifier("ng-controller", Model.of(this.getScope())));
	}
